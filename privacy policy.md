...
- Your API key (for accessing the selected LLM API, defaultly Google Gemini API)
...
## Third-Party API
As mentioned, the extension interacts with a third-party API to generate informative headlines. The default LLM API is Google Gemini, but if selected it can instead make requests to Groq, OpenAI (ChatGPT), or Claude. When you click on the extension, it identifies headlines on a news website, extracts the content of the articles, and sends this content to the selected LLM API to generate new headlines. The data sent to the API includes:
...
If you use a provider's free tier (including Google Gemini's free tier), that provider's own terms may allow it to log and review your prompts and generated output to improve its products. Refer to your chosen provider's own privacy policy and terms of service for details, and avoid submitting sensitive or confidential content.
